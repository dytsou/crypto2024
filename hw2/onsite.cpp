...
class DES{
    ...
    int S[8][4][16] = {                        // S-box
        {
            { 13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7 }, // original s8 modified
            { 1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2 },
            { 7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8 },
            { 2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11 }            
        },
        {
            { 15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10 },//S2 not uses
        ...
        {
            { 14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7 },//original s1 modified
...
signed main(){
    for(int i = 0; i < 2; i++){// modified
        DES* des = new DES();
        cin >> des->plaintext >> des->key; // modified
        cout << des->crypt() << endl;
        delete des;
    }
    return 0;
...
string DES::get_s_box_element(const string &s, int n){
    if(n == 1){ // modified
        int i = (
            (s[0] - '0') * 2 + (s[1] - '0') * 3 + 
            (s[2] - '0') * 5 + (s[3] - '0') * 7 +
            (s[4] - '0') * 11 + (s[5] - '0') * 13 ) % 16;
        // cout << s << " " << i << endl;
        return dec_to_4bit_bin(i);
    }
